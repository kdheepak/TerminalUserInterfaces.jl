...
function split(layout::Union{Horizontal,Vertical}, area::Rect)
  ...
  # If there's any Min constraint, distribute the remaining length equally to all Min constraints
  remaining_length = total_space - sum(min_and_max_lengths)
  ...
  # Create subareas (chunks) based on computed lengths
  chunks = []
  for length in min_and_max_lengths
    rect =
